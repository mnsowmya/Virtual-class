...
function googlecap() {     
	...
	document.getElementById("googlecaptcha").style.display = "block";   
    
}
       
 function formValidate_cap()    
 	{
	...
	if(seperateCoursenew_form=='not_found'){
		...
		var uCaptcha = trim(document.register.recaptcha_response_field.value);
		var uCaptcha_ch = trim(document.register.recaptcha_challenge_field.value);
		if(uCaptcha =='')
		{
			alert("Type the characters you see in the picture ");
			document.register.recaptcha_response_field.focus();
			return false;
		}
		else
		 {
				...
				if(checked==1){   
					var params = "firstname="+uFname+"&lastname="+uLname+"&middlename="+uMname+"&email="+uEmail+"&password="+uPass1+"&repassword="+uPass2+"&currently="+uCurr+"&sex="+uSex+"&message="+uCaptcha+"&uphnum="+uPhone+"&c_id=both"+"&tcid="+total+"&country="+ucountry+"&message_ch="+uCaptcha_ch;
				}else{    
		 			var params = "firstname="+uFname+"&lastname="+uLname+"&middlename="+uMname+"&email="+uEmail+"&password="+uPass1+"&repassword="+uPass2+"&currently="+uCurr+"&sex="+uSex+"&message="+uCaptcha+"&uphnum="+uPhone+"&c_id="+cid+"&country="+ucountry+"&message_ch="+uCaptcha_ch;}
		...
		if (xmlHttp3.readyState==4 && xmlHttp3.status == 200)
		 {
			...
			if(ridmsg=='RES|Inserted' && rid1!='')
			...
			else if(xmlHttp3.responseText=='Wrong_Captcha')
				{
					...
					Recaptcha.reload();
					document.register.recaptcha_response_field.value='';
					document.register.recaptcha_response_field.focus();
