...
function contactCounselors_new(cc){
		//alert(document.contact_counselors_form.course.selectedIndex.value);
	...
	var captchaval_new = trim(document.contact_counselors_form_new.captcha_new.value);
	...
	if(user_name_new=="* Name" || user_name_new==""){
  	...
  	if(captchaval_new==""){
	  alert("Please type the characters you see in the picture.");
	  document.contact_counselors_form_new.captcha_new.focus();
	  return false;
  	}
  	else{
		 var params = "un="+user_name_new+"&ue="+user_email_new+"&up="+user_ph_new+"&uq="+escape(user_query_new)+"&capid="+captchaval_new+"&cid="+cid_new+"&cc="+cc;
		 ...
		 xmlHttp2new.onreadystatechange = function(){
			if (xmlHttp2new.readyState==4 && xmlHttp2new.status == 200){
				...
				if(xmlHttp2new.responseText=="RES|1"){
					...
					document.contact_counselors_form_new.captcha_new.value='';
					document.contact_counselors_form_new.captcha_new.focus();
